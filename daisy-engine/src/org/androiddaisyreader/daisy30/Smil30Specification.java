...
public class Smil30Specification extends DefaultHandler {
	...
	private void handleTextElement(Attributes attributes) {
		...
		String[] elements = Daisy30Snippet.parseCompositeReference(src);
		...
		partBuilder.addSnippet(new Daisy30Snippet(doc, id));
	}
	...
	private void handleAudio(Attributes attributes) {
		...
		int clipBegin = ExtractTimingValues30.extractTimingAsMilliSeconds("clipBegin", attributes);
		int clipEnd = ExtractTimingValues30.extractTimingAsMilliSeconds("clipEnd", attributes);
		String id = ParserUtilities.getValueForName("id", attributes);
